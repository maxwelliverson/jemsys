...
namespace impl{
  namespace {
    using message_t = agt::ipc_message*;
    using mailbox_t = agt::ipc_mpsc_mailbox*;


    JEM_forceinline void enqueue_slot(mailbox_t mailbox, jem_size_t messageSlot) noexcept {
      jem_size_t lastQueuedSlot = mailbox->lastQueuedSlot.load(std::memory_order_acquire);
      message_t  lastQueuedMsg;
      do {
        lastQueuedMsg = get_message(mailbox, lastQueuedSlot);
        lastQueuedMsg->nextSlot = messageSlot;
      } while ( !mailbox->lastQueuedSlot.compare_exchange_weak(lastQueuedSlot, messageSlot) );

      mailbox->queuedSinceLastCheck.fetch_add(1, std::memory_order_relaxed);
      mailbox->queuedSinceLastCheck.notify_one();
    }
    JEM_forceinline void enqueue_slots(mailbox_t mailbox, jem_size_t slotCount, jem_size_t firstMsgSlot, jem_size_t lastMsgSlot) noexcept {
      jem_size_t lastQueuedSlot = mailbox->lastQueuedSlot.load(std::memory_order_acquire);
      message_t  lastQueuedMsg;
      do {
        lastQueuedMsg = get_message(mailbox, lastQueuedSlot);
        lastQueuedMsg->nextSlot = firstMsgSlot;
      } while ( !mailbox->lastQueuedSlot.compare_exchange_weak(lastQueuedSlot, lastMsgSlot) );

      mailbox->queuedSinceLastCheck.fetch_add(slotCount, std::memory_order_relaxed);
      mailbox->queuedSinceLastCheck.notify_one();
    }
    JEM_forceinline void release_slot(mailbox_t mailbox, jem_size_t messageSlot) noexcept {
      jem_size_t newNextMsg = mailbox->nextFreeSlot.load(std::memory_order_acquire);
      message_t  message    = get_message(mailbox, messageSlot);
      do {
        message->nextSlot = newNextMsg;
      } while ( !mailbox->nextFreeSlot.compare_exchange_weak(newNextMsg, messageSlot));
      mailbox->slotSemaphore.release();
    }

    JEM_forceinline void*         message_to_payload(const agt::ipc_mpsc_mailbox* mailbox, agt_message_t message) noexcept {
      return reinterpret_cast<jem_u8_t*>(message) + mailbox->payloadOffset;
    }
    JEM_forceinline agt_message_t payload_to_message(const agt::ipc_mpsc_mailbox* mailbox, void* payload) noexcept {
      return reinterpret_cast<agt_message_t>(static_cast<std::byte*>(payload) - mailbox->payloadOffset);
    }


    JEM_forceinline void* get_free_slot(agt::ipc_mpsc_mailbox* mailbox) noexcept {
      jem_size_t messageSlot = mailbox->nextFreeSlot.load(std::memory_order_acquire);
      jem_size_t nextMsgSlot;
      message_t  message;

      do {
        message     = get_message(mailbox, messageSlot);
        nextMsgSlot = message->nextSlot;
      } while ( !mailbox->nextFreeSlot.compare_exchange_weak(messageSlot, nextMsgSlot) );

      return message_to_payload(mailbox, message);
    }

    JEM_forceinline void wait_on_queued_messages(agt::ipc_mpsc_mailbox* mailbox, jem_size_t count) noexcept {
    ...
    JEM_forceinline bool try_wait_on_queued_messages(agt::ipc_mpsc_mailbox* mailbox, jem_size_t count) noexcept {
    ...
    JEM_forceinline bool try_wait_on_queued_messages_for(agt::ipc_mpsc_mailbox* mailbox, jem_size_t count, jem_u64_t timeout_us) noexcept {
      if ( mailbox->minQueuedMessages < count ) {
...
extern "C" {
  ...
  JEM_api agt_status_t        JEM_stdcall agt_ipc_mpsc_mailbox_acquire_many_slots(agt_handle_t handle, jem_size_t slotSize, jem_size_t slotCount, void** slots) JEM_noexcept {
    ...
    assert( agt_internal_get_object_kind(handle) == agt::object_kind_ipc_mpsc_mailbox );
  ...
  JEM_api void                JEM_stdcall agt_ipc_mpsc_mailbox_send_many(agt_handle_t handle, jem_size_t slotCount, void** messageSlots, agt_message_t* messages, agt_send_message_flags_t flags) JEM_noexcept {
  ...
  JEM_api agt_status_t        JEM_stdcall agt_ipc_mpsc_mailbox_receive_many(agt_handle_t handle, jem_size_t count, agt_message_t* messages) JEM_noexcept {
    ...
    --mailbox->minQueuedMessages;
    agt_ipc_mpsc_mailbox_discard(mailbox->previousReceivedMessage);
    mailbox->previousReceivedMessage = message;
  ...
  JEM_api agt_status_t        JEM_stdcall agt_ipc_mpsc_mailbox_try_acquire_many_slots(agt_handle_t handle, jem_size_t slotSize, jem_size_t slotCount, void** slots, jem_u64_t timeout_us) JEM_noexcept {
    ...
    if ( timeout_us == JEM_WAIT ) [[unlikely]]
    return agt_ipc_mpsc_mailbox_acquire_many_slots(handle, slotSize, slotCount, slots);
