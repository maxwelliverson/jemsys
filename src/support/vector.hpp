...
#define JEM_empty_bases __declspec(empty_bases)
#else
#define JEM_empty_bases
#endif
...
namespace jem{
  ...
  namespace impl{
    ...
    protected:
      ...
      JEM_noinline void* malloc_for_grow(jem_u64_t minimum_size, jem_u64_t type_size, jem_u64_t& new_capacity, jem_u64_t alignment);
      JEM_noinline void  grow_pod(void *first_element, jem_u64_t minimum_capacity, jem_u64_t type_size, jem_u64_t alignment);
    ...
    public:
      ...
      JEM_nodiscard size_type size() const noexcept { return size_; }
      JEM_nodiscard size_type capacity() const noexcept { return capacity_; }

      JEM_nodiscard bool empty() const noexcept { return !size_; }

      void set_size(jem_u64_t new_size) noexcept {
        JEM_assert(new_size <= capacity());
        size_ = new_size;
      }
    };
    ...
    class small_array_template_common : public vector_base<small_array_size_type<T>>{
      ...
      JEM_nodiscard void* get_first_element() const noexcept {
    ...
    protected:
      ...
      JEM_nodiscard bool is_small() const noexcept {
      ...
      void assert_safe_to_reference_after_resize(const void *element, jem_u64_t NewSize) noexcept {
        JEM_assert(is_safe_to_reference_after_resize(element, NewSize) &&
    ...
    public:
      ...
      JEM_nodiscard iterator       begin()        noexcept { return (iterator)this->begin_; }
      JEM_nodiscard const_iterator begin()  const noexcept { return (const_iterator)this->begin_; }
      JEM_nodiscard const_iterator cbegin() const noexcept { return this->begin(); }

      JEM_nodiscard iterator       end()        noexcept { return begin() + size(); }
      JEM_nodiscard const_iterator end()  const noexcept { return begin() + size(); }
      JEM_nodiscard const_iterator cend() const noexcept { return this->end(); }

      // reverse iterator creation methods.
      JEM_nodiscard reverse_iterator       rbegin()        noexcept { return reverse_iterator(end()); }
      JEM_nodiscard const_reverse_iterator rbegin()  const noexcept { return const_reverse_iterator(end()); }
      JEM_nodiscard const_reverse_iterator crbegin() const noexcept { return this->rbegin(); }

      JEM_nodiscard reverse_iterator       rend()        noexcept { return reverse_iterator(begin()); }
      JEM_nodiscard const_reverse_iterator rend()  const noexcept { return const_reverse_iterator(begin()); }
      JEM_nodiscard const_reverse_iterator crend() const noexcept { return this->rend(); }

      JEM_nodiscard size_type size_in_bytes() const noexcept { return size() * sizeof(T); }
      JEM_nodiscard size_type max_size() const noexcept {
        constexpr static jem_u64_t max_elements = size_type(-1) / sizeof(T);
        return std::min(base::max_size(), max_elements);
      }

      JEM_nodiscard size_type capacity_in_bytes() const noexcept { return capacity() * sizeof(T); }

      /// Return a pointer to the vector's buffer, even if empty().
      JEM_nodiscard pointer data() noexcept { return pointer(begin()); }
      /// Return a pointer to the vector's buffer, even if empty().
      JEM_nodiscard const_pointer data() const noexcept { return const_pointer(begin()); }

      JEM_nodiscard reference operator[](size_type idx) noexcept {
        JEM_assert(idx < size());
        return begin()[idx];
      }
      JEM_nodiscard const_reference operator[](size_type idx) const noexcept {
        JEM_assert(idx < size());
        return begin()[idx];
      }

      JEM_nodiscard reference       front() noexcept {
        JEM_assert(!empty());
        return begin()[0];
      }
      JEM_nodiscard const_reference front() const noexcept {
        JEM_assert(!empty());
        return begin()[0];
      }

      JEM_nodiscard reference       back() noexcept {
        JEM_assert(!empty());
        return end()[-1];
      }
      JEM_nodiscard const_reference back() const noexcept {
        JEM_assert(!empty());
        return end()[-1];
      }
    };
  ...
  public:
    ...
    void pop_back_n(size_type NumItems) {
      JEM_assert(this->size() >= NumItems);
      this->destroy_range(this->end() - NumItems, this->end());
      this->set_size(this->size() - NumItems);
    }

    JEM_nodiscard T pop_back_val() {
    ...
    iterator erase(const_iterator CI) {
      ...
      JEM_assert(this->is_reference_to_storage(CI) && "Iterator to erase is out of bounds.");
    ...
    iterator erase(const_iterator CS, const_iterator CE) {
      ...
      JEM_assert(this->is_range_in_storage(S, E) && "Range to erase is out of bounds.");
  ...
  private:
    ...
    iterator insert_one_impl(iterator I, ArgType &&Elt) {
      ...
      JEM_assert(this->is_reference_to_storage(I) && "Insertion iterator is out of bounds.");
  ...
  class JEM_empty_bases vector : public any_vector<T>, impl::small_array_inline_storage<T, N>{
  public:
